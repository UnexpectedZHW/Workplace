<div class="page-content">
    ...
    <table id="sample-table-1" class="table table-striped table-bordered table-hover">
        ...
        <tbody>
            <?php
            $no = 1;
            foreach ($getSu as $data) {
                ?>
                <tr>
                    <td class="center"><?php echo $no ?></td>
                    <td><?php echo $data['permKey'] ?></td>
                    <td><?php echo $data['permName'] ?></td>
                    <td>
                        <a href="<?php echo site_url('su/dashboard/permission_edit') . '/' . $data['ID'] ?>" >
                            <button class="btn btn-xs btn-success"><i class="icon-edit" ></i> Edit</button>
                        </a> 
                        | 
                        <button class="btn btn-xs btn-danger" id="btnDelete" onclick="dlt(<?php echo $data['ID'].",'".$data['permKey']; ?>')"> <i class="icon-trash"></i> Delete</button>
                    </td>
                </tr>
                <?php
                $no++;
    ...
    <a href="<?php echo site_url('su/dashboard/permission_tambah') ?>">
    ...
    <script>
        function dlt(id, name){
                var sid = "<?php echo site_url("su/dashboard/permission_delete").'/'; ?>"+id;
